...
poly shift(poly p, int x){
  if (x >= 0)
    p.insert(begin(p), x, 0);
  else
    assert(sz(p) + x >= 0), p.erase(begin(p), begin(p) - x);
  return p;
}
...
poly &operator*=(poly &lhs, const T &rhs){
  for (auto &x : lhs)
    x *= rhs;
  return lhs;
}
...
poly sqrt(const poly &p, int n){
  ...
  poly q{1};
  for (int i = 2; i / 2 < n; i *= 2)
    q = invmod(2) * (q + rsz(rsz(p, i) * inv(q, i), i));
  return q;
}
